...
inflation_data['Month'] = inflation_data['Month'].str.strip()
inflation_data['Month'].replace({'Marcrh': 'March'}, inplace=True)
...
# Correlation heatmap
selected_columns = ['GDP_In_Billion_USD', 'Per_Capita_in_USD', 'Percentage_Growth', 'Estimated Unemployment Rate (%)', 'General index']
...
# Distributions
for col in ['GDP_In_Billion_USD', 'Estimated Unemployment Rate (%)', 'General index']:
...
# Anomaly detection + event overlay
z_scores = np.abs(zscore(gdp_series))
anomalies = gdp_series[z_scores > 2.0]

plt.figure(figsize=(10, 6))
plt.plot(gdp_series, label='GDP')
plt.scatter(anomalies.index, anomalies, color='red', label='Anomalies')
plt.axvline(2020, color='gray', linestyle='--', label='COVID-19')
plt.axvline(2008, color='blue', linestyle='--', label='2008 Financial Crisis')
plt.title('GDP with Anomaly Detection and Event Markers')
plt.xlabel('Year')
